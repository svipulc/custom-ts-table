import {
  ...
  Header,
  HeaderGroup,
  createColumnHelperFn,
  myColumns,
} from "../types";
import { GetRowValue } from "./row";

// create column helper function
export function createColumnHelper<T>(): createColumnHelperFn<T> {
  return {
...
export function createHeaderGroups<T>(
  ...
  function processColumn(column: ColumnDef<T>, depth: number): Header<T> {
    ...
    if (column.columns) {
      header.subHeaders = column.columns.map(subColumn => processColumn(subColumn, depth + 1));
      header.colSpan = header.subHeaders.reduce((sum, subHeader) => sum + subHeader.colSpan, 0);
    } else {
  ...
  const allHeaders = columns.map(column => processColumn(column as ColumnDef<T>, 0));
